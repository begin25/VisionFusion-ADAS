...
class MetricsLogger:
    ...
    statistics, then produce a printable + CSV report at the end.

    Key metrics produced:
      - System latency (mean / p95 / max)
      - Effective FPS
      - Unique vehicles tracked + average track lifetime
      - Risk distribution (% CRITICAL / WARNING / SAFE)
      - EMA smoothing effectiveness: variance reduction in depth and TTC
    """

    def __init__(self):
        self.frame_log        = []   # one dict per processed frame
        self.track_lifetimes  = {}   # {track_id: frame_count}
        self.raw_dist_log     = {}   # {track_id: [raw_dist, ...]}
        self.smooth_dist_log  = {}   # {track_id: [smoothed_dist, ...]}
        self.raw_ttc_log      = []   # every raw instant-TTC value
        self.smooth_ttc_log   = []   # every EMA-smoothed TTC value
        self.alpha_dist = 0.3
        self.alpha_vel = 0.15
    ...
    def log_frame(self, frame_no, latency_ms, num_trackers,
                  risk_counts: dict):
        """
        risk_counts = {'CRITICAL': int, 'WARNING': int, 'SAFE': int}
        (counts of vehicles in each tier in THIS frame)
        """
    ...
    def log_vehicle(self, track_id, raw_dist, smoothed_dist,
        ...
        """
        raw_ttc / smoothed_ttc = float('inf') when no approach detected.
        We only log finite values (i.e., when a vehicle IS approaching).
        """
        tid = str(track_id)
    ...
    def generate_report(self, alpha_dist, alpha_vel, csv_path='frame_log.csv',
        ...
        latencies = [f['latency_ms'] for f in self.frame_log]
        lat_arr   = np.array(latencies)

        # Frames where at least one vehicle was in that tier
        critical_frames = sum(1 for f in self.frame_log if f['critical'] > 0)
        warning_frames  = sum(1 for f in self.frame_log if f['warning']  > 0
                                                        and f['critical'] == 0)
...
"""
        ...
        # Save per-frame CSV for any further analysis
        if self.frame_log:
...
class VisionFusionADAS:
    def __init__(self, yolo_model='yolov8n.pt'):
        ...
        self.alpha_dist = 0.3    # depth smoother
        self.alpha_vel  = 0.15   # velocity smoother (keeps TTC stable)

        # 4. Object history + metrics
        self.object_history = {}   # {track_id: {'dist', 'vel', 'time'}}
    ...
    def process_stream(self, input_path, output_path='fusion_output.mp4'):
        ...
        while cap.isOpened():
            ...
            results   = self.yolo.track(frame, classes=self.target_classes,
            ...
            track_ids = (results.boxes.id.cpu().numpy()
                         if results.boxes and results.boxes.id is not None else [])
            ...
            for i, box in enumerate(boxes):
                ...
                if t_id != -1:
                    if t_id in self.object_history:
                        ...
                        if dt > 0:
                            smoothed_dist = (raw_dist * self.alpha_dist +
                                             prev['dist'] * (1.0 - self.alpha_dist))

                            raw_vel       = (prev['dist'] - smoothed_dist) / dt

                            # Raw TTC for smoothing comparison
                            if raw_vel > 0.5:
                                raw_ttc = prev['dist'] / raw_vel

                            smoothed_vel = (raw_vel * self.alpha_vel +
                                            prev['vel'] * (1.0 - self.alpha_vel))
                ...
                # ── Log this vehicle ───────────────────────────────────
                if t_id != -1:
                    self.metrics.log_vehicle(t_id, raw_dist, smoothed_dist,
                                             raw_ttc, ttc)
            ...
            # ── 4. MiDaS PiP ────────────────────────────────────────────
            disp_norm    = cv2.normalize(disparity_map, None, 0, 255,
            ...
            cv2.rectangle(frame, (width-pip_w-20, 20), (width-20, 20+pip_h),
                          (255, 255, 255), 2)
            cv2.putText(frame, "Live MiDaS Depth Radar",
                        (width-pip_w-15, pip_h + 40),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 2)

            # ── 5. Telemetry HUD ─────────────────────────────────────────
            latency = (time.time() - current_time) * 1000
            cv2.putText(frame, f"Latency: {latency:.1f}ms | Trackers: {len(track_ids)}",
